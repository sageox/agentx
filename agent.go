...
// enabling generic event handling across different coding agents.
...
// StampComment returns the HTML comment prefix for a given stamp prefix.
// Format: "<!-- <prefix>-hash: "
...
// Format: <!-- <prefix>-hash: <12-char-hash> ver: <version> -->
func StampedContent(content []byte, version, prefix string) []byte {
	hash := ContentHash(content)
	comment := StampComment(prefix)
	stamp := fmt.Sprintf("%s%s ver: %s -->\n", comment, hash, version)
...
func ShouldWriteCommand(existing []byte, cmd CommandFile, overwrite bool, prefix string) bool {
	...
	installedHash := ExtractCommandHash(existing, prefix)
	if installedHash == "" {
		return false
	}
	...
	installedVer := ExtractStampVersion(existing, prefix)
